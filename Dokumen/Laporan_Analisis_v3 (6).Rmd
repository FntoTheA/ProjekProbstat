...
## Ranking Kemakmuran Provinsi Berdasarkan Tren Waktu (2020--2024)

Untuk menilai kemakmuran secara lebih adil, analisis dilakukan per provinsi per tahun menggunakan **composite score** yang menggabungkan 5 indikator utama: PDRB per kapita, IPM, akses internet, kemiskinan (dibalik), dan pengangguran (dibalik). Setiap indikator dinormalisasi ke skala 0--1 per tahun sebelum digabungkan.

```{r trend-provinsi, echo=FALSE}
library(tidyr)

# Agregasi rata-rata per provinsi per tahun
trend_provinsi <- data_clean %>%
  group_by(provinsi, tahun) %>%
  summarise(
    PDRB_juta      = round(mean(pdrb_perkapita, na.rm = TRUE) / 1e6, 2),
    ...
    Akses_Internet = round(mean(akses_internet, na.rm = TRUE), 2),
    .groups = "drop"
  )

# Composite score: normalisasi per tahun
trend_score <- trend_provinsi %>%
  group_by(tahun) %>%
  mutate(
    s_pdrb     = (PDRB_juta      - min(PDRB_juta))      / (max(PDRB_juta)      - min(PDRB_juta)),
    s_ipm      = (IPM            - min(IPM))             / (max(IPM)            - min(IPM)),
    s_internet = (Akses_Internet - min(Akses_Internet))  / (max(Akses_Internet) - min(Akses_Internet)),
    s_miskin   = 1 - (Kemiskinan   - min(Kemiskinan))   / (max(Kemiskinan)   - min(Kemiskinan)),
    s_nganggur = 1 - (Pengangguran - min(Pengangguran)) / (max(Pengangguran) - min(Pengangguran)),
    composite  = round((s_pdrb + s_ipm + s_internet + s_miskin + s_nganggur) / 5, 3)
  ) %>%
  ungroup() %>%
  select(provinsi, tahun, composite) %>%
  pivot_wider(names_from = tahun, values_from = composite) %>%
  arrange(desc(`2024`))

# Tambah kolom tren (rata-rata semua tahun)
trend_score$Rerata <- round(
  rowMeans(trend_score[, as.character(2020:2024)], na.rm = TRUE), 3
)

knitr::kable(
  trend_score,
  caption  = "Composite Score Kemakmuran per Provinsi per Tahun (Skala 0--1, makin tinggi makin makmur)",
  booktabs = TRUE,
  align    = "lrrrrrr",
  col.names = c("Provinsi", "2020", "2021", "2022", "2023", "2024", "Rerata")
) %>%
  kable_styling(
    latex_options = c("striped", "hold_position", "scale_down"),
    font_size     = 9
...
```{r trend-plot, echo=FALSE, fig.cap="Tren Composite Score Kemakmuran per Provinsi (2020--2024)", fig.height=4.5}
trend_plot <- trend_provinsi %>%
  group_by(tahun) %>%
  mutate(
    s_pdrb     = (PDRB_juta      - min(PDRB_juta))      / (max(PDRB_juta)      - min(PDRB_juta)),
    s_ipm      = (IPM            - min(IPM))             / (max(IPM)            - min(IPM)),
    ...
    s_miskin   = 1 - (Kemiskinan   - min(Kemiskinan))   / (max(Kemiskinan)   - min(Kemiskinan)),
    s_nganggur = 1 - (Pengangguran - min(Pengangguran)) / (max(Pengangguran) - min(Pengangguran)),
    composite  = (s_pdrb + s_ipm + s_internet + s_miskin + s_nganggur) / 5
  ) %>%
  ungroup()

ggplot(trend_plot, aes(x = tahun, y = composite,
                       color = provinsi, group = provinsi)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2) +
  scale_x_continuous(breaks = 2020:2024) +
  labs(
    title  = "Tren Composite Score Kemakmuran per Provinsi (2020--2024)",
    x      = "Tahun",
...
**Interpretasi**: Tabel dan grafik di atas menunjukkan perkembangan skor kemakmuran relatif tiap provinsi dari tahun ke tahun. Provinsi dengan skor **rerata tertinggi** merupakan provinsi paling makmur secara keseluruhan dalam periode 2020--2024, sedangkan provinsi dengan skor **rerata terendah** adalah yang paling perlu mendapat perhatian pembangunan.
...
## Ranking Kemakmuran Provinsi

```{r trend-provinsi, echo=TRUE, eval=FALSE}
```

```{r trend-plot, echo=TRUE, eval=FALSE}
```
